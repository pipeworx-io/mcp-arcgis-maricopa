 ...
 *
 * Maricopa County GIS (ArcGIS org ykpntM6e3tHvzKRJ): parcels, elections, public works & planning. search_datasets finds layers (each
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Arcgis Maricopa');
}


const HUB = 'https://hub.arcgis.com/api/v3/datasets';
const UA = 'pipeworx-mcp-arcgis-maricopa/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Maricopa County GIS open geospatial datasets (parcels, elections, public works & planning) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (Maricopa County GIS).' },
